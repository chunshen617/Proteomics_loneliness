#Mediation analysis based on marginal structural models, using CVD as an example
library('data.table')
library('lubridate')                                        
library('RNOmni')
library('survival')
...
Result_CVD_cmed1 <- foreach(i=1:length(prot_use), .combine='rbind') %dopar% {
  ...
  ########
  #Cox Model
  fitCox_1 <- coxph(Surv(fultime, outcome)~factor(LO_2c)+factor(LOstar)+age+sex,
                    data=newDat_simple, weights=newDat_simple$weight, cluster=newDat_simple$eid)
  sumCox_1 <- summary(fitCox_1)
  ...
  result <- data.frame(dirEff=result_Aalen_1[1,2],dirSE=result_Aalen_1[1,3],dirRSE=result_Aalen_1[1,4],dirZ=result_Aalen_1[1,5],dirP=result_Aalen_1[1,6],
                       ...
                       dirHR=sumCox_1[["coefficients"]][1,2],dirCox_RSE=sumCox_1[["coefficients"]][1,4],dirCox_P=sumCox_1[["coefficients"]][1,6],
                       dirCox_lower=sumCox_1[["conf.int"]][1,3],dirCox_upper=sumCox_1[["conf.int"]][1,4],
                       indirHR=sumCox_1[["coefficients"]][2,2],indirCox_RSE=sumCox_1[["coefficients"]][2,4],indirCox_P=sumCox_1[["coefficients"]][2,6],
                       indirCox_lower=sumCox_1[["conf.int"]][2,3],indirCox_upper=sumCox_1[["conf.int"]][2,4])
